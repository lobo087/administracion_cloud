...
from schemas import (
    AvalRespuesta,
    TituloBlockchain,
    TituloBlockchainConCodigo,
    UniversidadBlockchain,
    UniversidadCrear,
    UniversidadHistorialRespuesta,
)
...
@app.get("/universidades", response_model=list[UniversidadBlockchain])
def listar_universidades():
    try:
        return blockchain.list_universities()
    except Exception as error:
        raise HTTPException(status_code=500, detail=str(error)) from error


@app.get("/universidades/{address}", response_model=UniversidadBlockchain)
def obtener_universidad(address: str):
    try:
        universidad = blockchain.get_university(address)
    except Exception as error:
        raise HTTPException(status_code=500, detail=str(error)) from error

    if not universidad["existe"]:
        raise HTTPException(status_code=404, detail="Universidad no registrada")
...
@app.get("/universidades-historial", response_model=list[UniversidadHistorialRespuesta])
def listar_historial_universidades(db: Session = Depends(get_db)):
    return db.query(models.UniversidadMinisterio).order_by(models.UniversidadMinisterio.id).all()
